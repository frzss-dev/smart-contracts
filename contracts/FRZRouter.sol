...
library FRZLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"6850ef3336f00e539578f6f7f29501b8b4bb9312dee7ac391bc574e426235a69" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IFRZPair(
    ...
    function quote(
        ...
        require(amountA > 0, "FRZLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "FRZLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "FRZLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "Library: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 fee = IFRZRouter(router).transactionFee(
                path[i],
                path[i + 1],
...
contract FRZRouter is IFRZRouter {
    using SafeMath for uint256;

    address public immutable override factory;
    address public immutable override WETH;
    address internal immutable FRZToken;
     ...
     * address - What is FRZ Token address?
     * uint256[] memory - List of FRZ Token balance values, that should correspond list of fees
     ...
     **** example **** Then user with 400000 FRZ Tokens will have 9970 fee level
     **** example **** and user with 1000000 FRZ Tokens will have 10000 fee level (0% fee)
     *
     * Function creates new contract with addresses of factory, WETH and FRZ Token
    ...
    function _addLiquidity(
        ...
        if (IFRZFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IFRZFactory(factory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = FRZLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = FRZLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint256 amountAOptimal = FRZLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
        ...
        )
    {
        ...
        address pair = FRZLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IFRZPair(pair).mint(to);
    }
        ...
        )
    {
        ...
        address pair = FRZLibrary.pairFor(factory, token, WETH);
    ...
    {
        address pair = FRZLibrary.pairFor(factory, tokenA, tokenB);
        IFRZPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint256 amount0, uint256 amount1) = IFRZPair(pair).burn(to);
        (address token0, ) = FRZLibrary.sortTokens(tokenA, tokenB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = FRZLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2
                ? FRZLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            IFRZPair(FRZLibrary.pairFor(factory, input, output))
    ...
    function swapExactTokensForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = FRZLibrary.getAmountsOut(
            factory,
            amountIn,
            path,
            address(this)
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            FRZLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = FRZLibrary.getAmountsIn(
            factory,
            amountOut,
            path,
            address(this)
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = FRZLibrary.sortTokens(input, output);
            IFRZPair pair = IFRZPair(
                FRZLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = FRZLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput,
                    swapFee
    ...
    function quote(
        ...
        return FRZLibrary.quote(amountA, reserveA, reserveB);
    }
     ...
     * Checks balance of FRZ Token. If balance is entitled to discount, discounts
     */
    ...
    {
        uint256 fees;
        address lpToken = FRZLibrary.pairFor(
            factory,
            _tokenIn,
            _tokenOut
        );
        ...
        uint256 tokenBalance = IERC20(FRZToken).balanceOf(_msgSender);
